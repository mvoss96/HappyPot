...
namespace
{
	...
	/** The firmware version as BTHome's u24 (obj 0xF2): MAJOR.MINOR.PATCH.
	 * From <repo>/VERSION via Zephyr's app_version.h -- see src/version.hpp. */
	constexpr uint32_t FW_VERSION =
		(static_cast<uint32_t>(APP_VERSION_MAJOR) << 16) |
		(static_cast<uint32_t>(APP_VERSION_MINOR) << 8) |
		APP_PATCHLEVEL;
	...
	int publish(int moisture_percent, int32_t soil_mv, int32_t battery_mv, int battery_percent)
	{
		...
		const bt_data svc = BT_DATA(BT_DATA_SVC_DATA16, packet.serviceData(),
									static_cast<uint8_t>(packet.serviceDataSize()));
	...
	/** The net:: hook: unpack the cycle's reading into one advert. Runs on the loop's
	 * thread; advertise_once() is non-blocking, so the loop is never held up. */
	void reading_hook(const net::Reading &r)
	{
		(void)publish(r.soil.percent, r.soil.mv, r.battery.mv, r.battery.percent);
	}
...
int bthome_install(void)
{
	...
	net::set_hooks({
		.reading = reading_hook,
		.factory_reset = nullptr, // a beacon has no network to leave
		.pairing_open = nullptr,  // and nothing to pair with
	});
	/* No QR, no manual code, no join hint: a BTHome receiver just listens. */
	net::set_radio({"BTHome", nullptr, nullptr, nullptr});
