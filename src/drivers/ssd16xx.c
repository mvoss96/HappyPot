 ...
 *
 * Vendored from zephyr/drivers/display/ssd16xx.c with local modifications.
 * See src/drivers/README.md for origin and change summary.
 */
...
static int ssd16xx_update_display(const struct device *dev)
{
	...
	const uint8_t update_cmd =
		...
		(partial ? 0 : (SSD16XX_CTRL2_DISABLE_ANALOG |
...
/* Local patch: GxEPD2-style hibernate. SUSPEND -> deep sleep mode 1.
 * RESUME -> HW reset + minimal re-init (LUT/orientation, no clear+update
 * so the next app write can be a partial refresh without a white flash).
 */
static int ssd16xx_resume(const struct device *dev)
{
	...
	/* Force profile re-upload: chip is fresh, but data->profile cache isn't. */
	data->profile = SSD16XX_PROFILE_INVALID;

	/* set_profile re-writes ENTRY_MODE using cached scan_mode, so
	 * orientation is restored implicitly.
	 */
