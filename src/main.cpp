...
namespace
{
    ...
    const gpio_dt_spec cal_btn   = GPIO_DT_SPEC_GET(DT_PATH(zephyr_user), cal_button_gpios);
    ...
    void sample_and_publish()
    {
        ...
        bthome_publish(soil_reading.percent, soil_reading.mv,
                       battery_reading.mv, battery_reading.percent);
    }
...
int main()
{
    ...
    if (calib_init(soil, cal_btn, &measurement_work) < 0)
    {
        LOG_WRN("calib init failed — running without calibration button");
    }
